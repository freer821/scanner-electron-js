...
function saveAndRemoveCurrentImageDetails() {
  ...
  zoomOutObjectOnImageSelect(
    images[currentlySelectedImageId].shapes,
    images[currentlySelectedImageId].labels,
...
function highlightMachineLearningButton() {
  if (!getHasMachineLearningButtonBeenHighligtedState()) {
    ...
    initiateButtonPulseAnimation(
      document.getElementById('machine-learning-button'),
      'rgb(184 233 179)',
...
function changeToExistingImage(id) {
  ...
  repopulateLabelAndShapeObjects(
    images[id].shapes,
    images[id].labels,
    images[id].imageDimensions,
    images[id].data,
  );
